...
void Screen::Screenshot() {
  ...
  contextRef = CGBitmapContextCreate(
      imageOriginal->data, imageOriginal->cols, imageOriginal->rows,
      8, imageOriginal->step[0],
      colorSpace, kCGImageAlphaPremultipliedLast | kCGBitmapByteOrderDefault);

  screenshotRef = CGDisplayCreateImageForRect(CGMainDisplayID(), CGRectMake(*windowX, *windowY, *width, *height));
